...
class CoTrackerFSPTRefiner(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.relocalization_feature_source not in ("cotracker", "geo"):
            logger.warning(
        ...
        self.relocalization_downsample = _as_int(_cfg_get(reloc_cfg, "downsample", 2), 2)
        self.relocalization_downsample = 1 if self.relocalization_downsample < 1 else self.relocalization_downsample
        self.relocalization_min_occlusion_len = _as_int(_cfg_get(reloc_cfg, "min_occlusion_len", 20), 20)
        self.relocalization_frames_after = _as_int(_cfg_get(reloc_cfg, "frames_after", 3), 3)
        self.relocalization_vis_threshold = _as_float(_cfg_get(reloc_cfg, "vis_threshold", 0.5), 0.5)
        self.relocalization_template_mode = str(_cfg_get(reloc_cfg, "template_mode", "query")).lower().strip()
        if self.relocalization_template_mode in ("", "default"):
            self.relocalization_template_mode = "query"
        if self.relocalization_template_mode not in ("query", "last_visible"):
            logger.warning(
                "Unsupported refiner.relocalization.template_mode="
                f"{self.relocalization_template_mode!r}; falling back to 'query'."
            )
            self.relocalization_template_mode = "query"
        self.relocalization_template_bank_size = _as_int(_cfg_get(reloc_cfg, "template_bank_size", 1), 1)
        ...
        self.relocalization_template_bank_stride = _as_int(_cfg_get(reloc_cfg, "template_bank_stride", 1), 1)
        if self.relocalization_template_bank_stride < 1:
            self.relocalization_template_bank_stride = 1
        if self.relocalization_template_bank_size > 1 and self.relocalization_template_mode != "last_visible":
            logger.warning(
                "refiner.relocalization.template_bank_size>1 is only supported when "
                "template_mode='last_visible'; disabling template bank."
            )
            self.relocalization_template_bank_size = 1
    ...
    def _compute_global_relocalization_step(
        ...
        *,
        ...
        # Project map/query features using the same projection as local_correlation.
    ...
    def _apply_trainability_from_config(self, refiner_cfg) -> None:
        ...
        if trainable_list is not None and len(trainable_list) > 0:
            ...
            for name, module in candidates.items():
                if module is None:
                    continue
                self._set_submodule_trainable(name, module, trainable=(name in trainable))
            ...
            for name, module in candidates.items():
                if module is None or name not in frozen:
                    continue
                self._set_submodule_trainable(name, module, trainable=False)
                ...
                if isinstance(relocal_feature_map, torch.Tensor) and relocal_feature_map.dim() == 5:
                    ...
                    if (
                        ...
                        try:
                            ...
                            bank_size = int(self.relocalization_template_bank_size)
                            bank_stride = int(self.relocalization_template_bank_stride)
                            bank_stride = 1 if bank_stride < 1 else bank_stride

                            if bank_size <= 1:
                                template_t = query_t_cpu.clone()
                                for b in range(B):
                                    for n in range(N):
                                        ...
                                        t_sel = tq
                                        for tt in range(first - 1, tq - 1, -1):
                                            if bool(visible[tt]):
                                                t_sel = tt
                                                break
                                        template_t[b, n] = t_sel

                                template_t = template_t.to(device=video.device, dtype=query_t.dtype)
                                gather_idx = template_t.view(B, 1, N, 1).expand(
                            ...
                            logger.warning(
                                f"Failed to compute last-visible relocal templates; falling back to query template: {exc}"
                            )
                            template_mode = "query"
