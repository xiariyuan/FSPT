...
def deform_inputs(vit_size, adapter_size, device):
    ...
    reference_points = get_reference_points([(h_vit // 14, w_vit // 14)], device)
    deform_inputs1 = [reference_points, spatial_shapes, level_start_index]


    spatial_shapes = torch.as_tensor([(h_vit // 14, w_vit // 14)], dtype=torch.long, device=device)
    level_start_index = torch.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
...
class InteractionBlockWithCls(nn.Module):
    ...
    def forward(self, x, c, cls_token, blocks, deform_inputs1, deform_inputs2, H, W):
        x = self.injector(query=x, reference_points=deform_inputs1[0],
                          feat=c, spatial_shapes=deform_inputs1[1],
                          level_start_index=deform_inputs1[2])
        x = torch.cat((cls_token, x), dim=1)  # [b, 1 + 4 + n, c]
