...
P0g active: move learning before candidate discretization with a causal multi-memory dense correlation proposal generator (CMCP). Preserve the frozen CoTracker3 backbone and native candidate, but train fit-only query/previous/EMA correlation maps with a causal ConvGRU heatmap proposal head. Require exact native-safe initialization, deterministic proposal export, candidate oracle >= +3.0 AJ, direct top-1 >= +0.5 AJ with positive paired CI, and no tail regression before any MUSR selector is reopened.
```
