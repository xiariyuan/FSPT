...
P0c active: the independent adapter-export replay check is complete with exact hashes and zero tensor differences. Now preregister and build disjoint multi-video Kubric candidate caches and require partition-level oracle headroom before MUSR training. CoTracker3 remains the first backbone; TAPNext/TAPNext++ or Track-On2 is the second-backbone validation route. Do not open a new external result until deterministic candidate export, Kubric multi-video qualification, learned Kubric gain, closed-loop safety, and a new preregistration all pass. Preserve the corrected Kinetics result unchanged and do not read it for tuning.
```
