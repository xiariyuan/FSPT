...
P0f active: keep the raw-v1 features and frozen candidates, but add one fixed causal six-frame, rank-invariant set-evidence accumulator. Beneficial global candidates occur on 37.63% of validation rows, persist with P(next beneficial | beneficial)=69.31% versus 18.65% after non-beneficial rows, and the single-frame raw selector recalls only 8.27%. Train selector-only on fit and select on model-validation; keep calibration, final_holdout, DAVIS, and Kinetics locked and do not sweep temporal windows or gates.
```
