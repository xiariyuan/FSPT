...
class TAPVidKineticsDataset(Dataset):
    ...
    Args:
        root: 数据集根目录，应包含 tapvid_kinetics.pkl 或 *_of_*.pkl 分片
    ...
    def _load_annotations(self, root: str) -> Dict:
        ...
        if os.path.exists(anno_path):
            with open(anno_path, 'rb') as f:
                data = pickle.load(f)
            return self._coerce_annotation_mapping(data, "tapvid_kinetics")
    ...
    def _generate_queries_from_tracks(
        ...
        if use_pixel_scale:
            ...
            points_xy_norm = points_xy / np.array([w_est, h_est], dtype=np.float32)
        else:
            h_est, w_est = 1.0, 1.0
        ...
        if use_pixel_scale:
            target_yx = target_yx * np.array([h_est, w_est], dtype=np.float32)
            query_yx = query_yx * np.array([h_est, w_est], dtype=np.float32)
