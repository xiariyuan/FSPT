...
if [[ "$#" -eq 0 ]]; then
  echo "Usage: bash projects/mmp_tracker/run_topk_candidate_v2_matrix.sh <seed> [seed ...]" >&2
  exit 1
fi
...
for seed in "$@"; do
  ...
  launch_one "projects/mmp_tracker/configs/localglobal_topk_abstain_v2_dev.yaml" "mmp_localglobal_topk_abstain_v2_dev" "$seed" "$ts"
done
