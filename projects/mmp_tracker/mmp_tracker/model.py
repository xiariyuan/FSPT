...
from .global_relocator import GlobalRelocator, PairwiseGlobalRelocator
...
class MMPTracker(nn.Module):
    ...
    def __init__(self, config: Optional[MMPTrackerConfig] = None):
        ...
        elif encoder_type in {"timm", "dinov2", "timm_dinov2"}:
            self.encoder = TimmFeatureEncoder(
                in_channels=enc_cfg.in_channels,
                out_dim=enc_cfg.out_dim,
                backbone_name=enc_cfg.backbone_name,
                pretrained=bool(enc_cfg.pretrained),
                pretrained_path=getattr(enc_cfg, "pretrained_path", ""),
                train_backbone=bool(enc_cfg.train_backbone),
                out_indices=tuple(enc_cfg.out_indices),
                fuse_mode=enc_cfg.fuse_mode,
            )
        else:
            raise ValueError(f"Unsupported encoder type: {enc_cfg.type}")
        ...
        global_relocator_type = str(
            getattr(self.config.global_relocator, "type", "memory_similarity") or "memory_similarity"
        ).strip().lower()
        if global_relocator_type in {"memory_similarity", "similarity", "legacy"}:
        ...
        elif global_relocator_type in {"pairwise", "pairwise_global_matcher"}:
            self.global_relocator = PairwiseGlobalRelocator(
                feature_dim=enc_cfg.out_dim,
                topk=self.config.global_relocator.topk,
                temperature=self.config.global_relocator.temperature,
                hidden_dim=self.config.global_relocator.hidden_dim,
                position_bias_strength=self.config.global_relocator.position_bias_strength,
                position_bias_sigma=self.config.global_relocator.position_bias_sigma,
            )
        else:
            raise ValueError(f"Unsupported global relocator type: {self.config.global_relocator.type}")
        ...
        for t in range(time):
            ...
            if self.variant == "local":
                ...
                if global_out.points.shape[2] > 0:
                    ...
                    oracle_rematch_candidate_points = refined_points
                ...
                if global_out.points.shape[2] > 0:
                    global_top1 = global_out.points[:, :, 0, :]
                    global_output_mode = str(
                        getattr(self.config.tracking, "global_output_mode", "top1_refine") or "top1_refine"
                    ).strip().lower()
                    ...
                    if global_output_mode in {"coarse_expected", "expected", "expected_coarse"}:
                        global_refine_out = LocalMatchOutput(
                            points=global_out.expected_points,
                            confidence=global_out.confidence,
                            entropy=global_out.entropy,
                            heatmap=global_out.heatmap,
                        )
                        global_points = global_out.expected_points
                        global_conf = global_quality
                        global_branch_conf = global_conf
                        global_branch_entropy = global_out.entropy
                        global_commit_reference = global_out.expected_points
                        oracle_rematch_candidate_points = global_out.expected_points.unsqueeze(2)
                    elif global_output_mode in {"coarse", "coarse_top1", "top1_coarse", "coarse_only"}:
                        global_refine_out = LocalMatchOutput(
                            points=global_top1,
                            confidence=global_out.confidence,
                            entropy=global_out.entropy,
                        ...
                        global_points = global_top1
                        global_conf = global_quality
                        global_branch_conf = global_conf
                        global_branch_entropy = global_out.entropy
                        global_commit_reference = global_top1
                        oracle_rematch_candidate_points = global_top1.unsqueeze(2)
                    else:
                        ...
                        global_commit_reference = global_center
                        oracle_rematch_candidate_points = global_points.unsqueeze(2)
                    ...
                    local_global_agreement_px = torch.norm((global_points - local_out.points) * px_scale, dim=-1)
                    coarse_refine_consistency_px = torch.norm((global_points - global_commit_reference) * px_scale, dim=-1)
                    ...
                    commit_features = torch.stack(
                        [
                            ...
                            torch.norm(global_points - prev_prior_points, dim=-1),
                            torch.norm(global_points - local_out.points, dim=-1),
                            torch.norm(global_points - global_commit_reference, dim=-1),
                        ],
                        dim=-1,
                    )
            ...
            memory_write_mode = str(
                getattr(self.config.tracking, "memory_write_mode", "append_visible") or "append_visible"
            ).strip().lower()
            if memory_write_mode not in {"frozen_initial", "freeze_initial", "initial_only", "none"}:
