...
if [[ "$#" -eq 0 ]]; then
  echo "Usage: bash projects/mmp_tracker/run_controlled_commit_matrix.sh <seed> [seed ...]" >&2
  exit 1
fi
...
for seed in "$@"; do
  ...
  launch_one "projects/mmp_tracker/configs/localglobal_top1_deferredcommit_dev.yaml" "mmp_localglobal_top1_deferredcommit_dev" "$seed" "$ts"
done
