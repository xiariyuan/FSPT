...
def run_family(root: Path, fam: str, tag: str) -> Dict[str, Any]:
    caches = {m: torch.load(cache_path(root, fam, m, tag), map_location='cpu', weights_only=False) for m in ['offline','online','b2_w16_p2']}
    ...
    return {'family': fam, 'L': 16, 'n_videos': len(perv['offline']), 'comparisons': comps}
...
def main() -> None:
    ...
    out = Path(args.output_json) if args.output_json else root / f'{args.tag}_statistical_robustness_summary.json'
    out.write_text(json.dumps({'split': args.tag, 'results': results}, indent=2, ensure_ascii=False))
    compact = []
    for r in results:
