...
OUT = ROOT / 'fresh20_29_frozen_validation_summary.json'
...
def cache_path(fam: str, model: str) -> Path:
    d = ROOT / f'{fam}_L16' / 'predictions'
    return d / f'cotracker3_{model}_{fam}_L16_fresh20_29.pt'


def b2_cache_path(fam: str) -> Path:
    return ROOT / f'{fam}_L16' / 'predictions' / f'b2_w16_p2_{fam}_L16_fresh20_29.pt'
...
def run_family(fam: str) -> Dict[str, Any]:
    off = torch.load(cache_path(fam, 'offline'), map_location='cpu', weights_only=False)
    on = torch.load(cache_path(fam, 'online'), map_location='cpu', weights_only=False)
    check_alignment(off, on)
    b2_path = b2_cache_path(fam)
    trigger_stats = build_b2(off, on, b2_path, name=f'b2_w16_p2_{fam}_L16_fresh20_29')
    rows = [
        eval_one('offline', cache_path(fam, 'offline')),
        eval_one('online', cache_path(fam, 'online')),
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({'split': 'rgb_fresh20_29', 'protocol': 'frozen_L16', 'results': results}, indent=2, ensure_ascii=False))
    compact = []
    for r in results:
        compact.append({
            'family': r['family'],
