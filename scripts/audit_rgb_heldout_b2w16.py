...
OUT = ROOT / 'b2_w16_heldout10_per_video_audit'
CACHES = {
...
def main() -> None:
    ...
    counts = {
        ...
        'b2_beats_online_AJ_by_10': count(lambda r: r['deltas']['b2_vs_online_AJ_256'] is not None and r['deltas']['b2_vs_online_AJ_256'] >= 10.0),
    }
    ...
    aggregate = {
        ...
        for name in ['offline', 'online', 'b2_w16']
    }
    ...
    summary = {
        ...
        'counts': counts,
        'best_b2_gain_vs_offline': sorted(rows, key=lambda r: r['deltas']['b2_vs_offline_AJ_RD_256'], reverse=True)[:5],
        'worst_b2_delta_vs_offline': sorted(rows, key=lambda r: r['deltas']['b2_vs_offline_AJ_RD_256'])[:5],
        'worst_b2_AJ_drop_vs_offline': sorted(rows, key=lambda r: r['deltas']['b2_vs_offline_AJ_256'])[:5],
        'rows': rows,
    }
