...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--config',required=True); p.add_argument('--checkpoint',required=True); p.add_argument('--selector-bundle',required=True); p.add_argument('--threshold',type=float,required=True); p.add_argument('--fusion-strength',type=float,default=None); p.add_argument('--max-switch-distance-px',type=float,default=None); p.add_argument('--split',default='train'); p.add_argument('--dataset-root',required=True); p.add_argument('--annotation-file',required=True); p.add_argument('--dataset-split',default='validation'); p.add_argument('--limit',type=int,default=32); p.add_argument('--device',default='cuda'); p.add_argument('--seed',type=int,default=17); p.add_argument('--output',required=True); args=p.parse_args(); random.seed(args.seed); np.random.seed(args.seed); torch.manual_seed(args.seed)
 ...
 result={'evidence_tier':'development_diagnostic_only','paper_claim_eligible':False,'evaluation_mode':'open_loop_posthoc_selector','closed_loop_state_updated':False,'threshold':args.threshold,'seed':args.seed,'fusion_strength':args.fusion_strength,'max_switch_distance_px':args.max_switch_distance_px,'samples':len(rows),'aggregate':agg,'delta_routeD_vs_baseline':delta,'per_sample':rows}
