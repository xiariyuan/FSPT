...
OUT = ROOT / 'translate_severity_summary.json'
...
def eval_L(L: int) -> Dict[str, Any]:
    ...
    pred = d / 'predictions'
    off = pred / f'cotracker3_offline_translate_L{L}.pt'
    on = pred / f'cotracker3_online_translate_L{L}.pt'
    b2 = pred / f'b2_w16_p2_translate_L{L}.pt'
...
def main() -> None:
    ...
    OUT.write_text(json.dumps({'stress_family': 'translate_exit_reenter', 'summaries': all_summaries}, indent=2, ensure_ascii=False))
    print(json.dumps({'stress_family': 'translate_exit_reenter', 'summaries': all_summaries}, indent=2, ensure_ascii=False), flush=True)
