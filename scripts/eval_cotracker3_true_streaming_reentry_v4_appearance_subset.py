...
DEFAULT_OUTDIR = ROOT / 'outputs/paper_discovery_2026-07-05/cotracker3_true_streaming_v4_appearance_subset_eval'
...
def safe_recovery_mask(
    ...
    *,
    ...
    appearance_scores: dict[tuple[int, int], float] | None = None,
    appearance_threshold: float | None = None,
    appearance_direction: str = 'high_is_good',
):
            ...
            for t in range(lo, s):
                ...
                if math.isfinite(max_jump_px):
                    ...
                    if jump > float(max_jump_px):
                        ...
                        continue
                if appearance_scores is not None and appearance_threshold is not None:
                    key = (int(q), int(t))
                    if key not in appearance_scores:
                        stats['missing_appearance'] += 1
                        continue
                    aval = float(appearance_scores[key])
                    ok = aval >= float(appearance_threshold) if appearance_direction == 'high_is_good' else aval <= float(appearance_threshold)
                    if not ok:
                        stats['rejected_appearance'] += 1
                        continue
                cand.append(t)
            if not cand:
                continue
...
def run_variant(model: CoTrackerOnlinePredictor, base_record: dict, video: torch.Tensor, queries_xy: torch.Tensor, variant: str, params: dict):
    ...
    V3 decouples internal state writeback from final output:
    - after each native forward, copy the pre-writeback prediction into an output buffer;
    - then optionally edit online_vis_predicted / online_conf_predicted for future windows;
    - final record is built from the output buffer, not directly from the edited state.
    """
    T = int(video.shape[1])
    ...
    video_id = str(base_record['video_id'])
    appearance_by_video = params.get('appearance_by_video') or {}
    appearance_scores_video = appearance_by_video.get(video_id)
    state_stats_total = {'candidate_frames': 0, 'opened_frames': 0, 'rejected_jump': 0, 'rejected_short': 0, 'rejected_score': 0, 'rejected_trend': 0, 'rejected_unstable_reentry': 0, 'rejected_not_allowed': 0, 'rejected_appearance': 0, 'missing_appearance': 0}
    out_coords = np.zeros((N, T, 2), dtype=np.float32)
    out_score = np.zeros((N, T), dtype=np.float32)
    out_vis = np.zeros((N, T), dtype=bool)
    copied = np.zeros((N, T), dtype=bool)
    with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.float16):
        for ind in range(0, T - model.step, model.step):
            ...
            # Copy pre-writeback outputs. Future windows may overwrite overlap frames with
            # predictions influenced by earlier state edits; direct edits from this step are
            # not counted as output unless a later forward predicts them.
            out_coords[:cur_N, :cur_T] = coords_yx_nt[:cur_N, :cur_T]
            out_score[:cur_N, :cur_T] = score_nt[:cur_N, :cur_T]
            out_vis[:cur_N, :cur_T] = base_vis_nt[:cur_N, :cur_T]
            copied[:cur_N, :cur_T] = True
            if variant in ('state_writeback', 'state_plus_output'):
                allowed_frames = set(range(ind + model.step, min(ind + 2 * model.step, cur_T)))
                open_mask_nt, st = safe_recovery_mask(score_nt, coords_yx_nt, base_vis_nt, tau_low=params['tau_low'], pre_window=params['pre_window'], max_jump_px=params['max_jump_px'], min_len=params['min_len'], confirm_visible_len=params['confirm_visible_len'], trend_min=params['trend_min'], allowed_frames=allowed_frames, appearance_scores=appearance_scores_video, appearance_threshold=params.get('appearance_threshold'), appearance_direction=params.get('appearance_direction', 'high_is_good'))
    ...
    # If any tail frames were not copied due to very short videos, fall back to final state.
    if not copied.all():
        coords_yx_nt, score_nt, base_vis_nt = state_to_arrays(model, T, N)
        miss = ~copied
        out_coords[miss] = coords_yx_nt[miss]
        out_score[miss] = score_nt[miss]
        out_vis[miss] = base_vis_nt[miss]
    output_stats = {'candidate_frames': 0, 'opened_frames': 0, 'rejected_jump': 0, 'rejected_short': 0, 'rejected_score': 0, 'rejected_trend': 0, 'rejected_unstable_reentry': 0, 'rejected_not_allowed': 0, 'rejected_appearance': 0, 'missing_appearance': 0}
    pred_vis_nt = out_vis.copy()
    if variant in ('output_only', 'state_plus_output'):
        open_mask_nt, output_stats = safe_recovery_mask(out_score, out_coords, pred_vis_nt, tau_low=params['tau_low'], pre_window=params['pre_window'], max_jump_px=params['max_jump_px'], min_len=params['min_len'], confirm_visible_len=params['confirm_visible_len'], trend_min=params['trend_min'], allowed_frames=None)
        pred_vis_nt[open_mask_nt] = True
    rec = make_record(base_record, out_coords, pred_vis_nt, out_score, f'cotracker3_true_streaming_{variant}')
    stats = {
...
def load_appearance_map(npz_path: str, feature: str) -> dict[str, dict[tuple[int, int], float]]:
    if not npz_path:
        return {}
    ...
    if feature not in names:
        raise KeyError(f'appearance feature {feature} not in {names}')
    j = names.index(feature)
    out: dict[str, dict[tuple[int, int], float]] = {}
    for mstr, val in zip(z['meta_json'].tolist(), X[:, j].tolist()):
        m = json.loads(str(mstr))
        vid = str(m['video_id'])
        key = (int(m['query_idx']), int(m['frame_t']))
        out.setdefault(vid, {})[key] = float(val)
    return out
...
def main():
    ...
    ap.add_argument('--appearance-npz', default='')
    ap.add_argument('--appearance-feature', default='last_strict_candidate_cosine')
    ap.add_argument('--appearance-threshold', type=float, default=0.7204042673110962)
    ap.add_argument('--appearance-direction', choices=['high_is_good','low_is_good'], default='high_is_good')
    args = ap.parse_args()
    if not torch.cuda.is_available():
        raise RuntimeError('CUDA is required')
    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    params = {'tau_low': args.tau_low, 'pre_window': args.pre_window, 'max_jump_px': args.max_jump_px, 'min_len': args.min_len, 'confirm_visible_len': args.confirm_visible_len, 'trend_min': args.trend_min, 'state_prob': args.state_prob, 'write_region': 'overlap_only_for_state', 'appearance_npz': args.appearance_npz, 'appearance_feature': args.appearance_feature, 'appearance_threshold': args.appearance_threshold if args.appearance_npz else None, 'appearance_direction': args.appearance_direction}
    params['appearance_by_video'] = load_appearance_map(args.appearance_npz, args.appearance_feature) if args.appearance_npz else {}
    ...
    for variant in variants:
        ...
        payload['schema_version'] = 'cotracker3_true_streaming_reentry_v4_appearance_subset_v1'
    ...
    safe_params = {k: v for k, v in params.items() if k != 'appearance_by_video'}
    if 'appearance_by_video' in params:
        safe_params['appearance_video_count'] = len(params.get('appearance_by_video') or {})
    report = {
        'script': 'scripts/eval_cotracker3_true_streaming_reentry_v4_appearance_subset.py',
        'params': safe_params,
    ...
    out_report = outdir / 'cotracker3_true_streaming_reentry_v4_appearance_subset_report.json'
