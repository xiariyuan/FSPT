...
IN=Path('outputs/paper_discovery_2026-06-27/b2wa_dinov3_dense_pilot/rgb_dev10_dinov3_dense_rows.jsonl')
OUT=Path('outputs/paper_discovery_2026-06-27/b2wa_dinov3_dense_pilot/dinov3_signal_summary.json')
...
def names(rs,kind):
 ds=sorted(rs[0]['dinov3_dense_features'].keys())
 if kind=='traj': return TRAJ
 if kind=='dinov3': return ['dino::'+k for k in ds]
 return TRAJ+['dino::'+k for k in ds]
def mat(rs,kind):
 ns=names(rs,kind); X=[]
 for r in rs:
  vals=[]
  for n in ns:
   vals.append(fnum(r['dinov3_dense_features'].get(n[6:])) if n.startswith('dino::') else fnum(r['traj_features'].get(n)))
  X.append(vals)
...
def model(rs,kind,t):
 ...
 for i,(tr,te) in enumerate(GroupKFold(n_splits=min(5,len(np.unique(g)))).split(X,y,g),1):
  clf=HistGradientBoostingClassifier(max_iter=140,learning_rate=.05,max_leaf_nodes=31,l2_regularization=.02,random_state=500+i)
...
def main():
 ...
 for t in targets:
  ...
  for kind in ['traj','dinov3','combined']:
