...
DEFAULT_OUTDIR=ROOT/'outputs/paper_discovery_2026-07-05/cotracker3_online_v6a3_reentry_utility'
...
def load(path,label):
    ...
    X=np.asarray(z['X'],np.float32); y=np.asarray(z[label],np.float32).astype(bool)
...
def models(seed):
    return {
        ...
        'extra_trees_balanced':ExtraTreesClassifier(n_estimators=500,max_depth=6,min_samples_leaf=2,class_weight='balanced',random_state=seed),
        'random_forest_balanced':RandomForestClassifier(n_estimators=500,max_depth=6,min_samples_leaf=2,class_weight='balanced',random_state=seed),
    }
...
def build(native,metas,p,thr):
    ...
    for i,m in enumerate(metas):
        if p[i]<thr: continue
        vi=vid_to_idx[m['video_id']]; q=int(m['query_idx']); t=int(m['frame_t'])
...
def main():
    ...
    ap.add_argument('--target',choices=['y_reentry_first','y_reentry_early2','y_reentry_early4','y_reentry_early8'],default='y_reentry_early4')
    ap.add_argument('--seed',type=int,default=0)
    args=ap.parse_args()
    outdir=Path(args.outdir); outdir.mkdir(parents=True,exist_ok=True)
    X,y,metas,names,groups=load(Path(args.labels),args.target)
    leak=[n for n in names if any(s in n.lower() for s in ['gt','err','safe','label','reentry'])]
    if leak: raise RuntimeError(f'leak features {leak}')
    native=torch.load(args.native_cache,map_location='cpu',weights_only=False)
    native_std=eval_records(native['records']); native_aj=run_ajrd(Path(args.native_cache),outdir/f'v6a3_native_ref_{args.target}_ajrd.json')
    native_row={**native_std,'AJ_RD':native_aj['true_AJ_RD'],'AJ_RD_256':native_aj['true_AJ_RD_256']}
    results={}; logo=LeaveOneGroupOut()
    for name,m in models(args.seed).items():
        ...
        np.save(outdir/f'v6a3_{name}_{args.target}_oof.npy',oof)
        thresholds=np.unique(np.concatenate([np.linspace(0.05,0.95,19),np.quantile(oof,np.linspace(0.75,0.995,12))])).astype(float)
        sweeps=[]
        for thr in thresholds:
            pred=oof>=thr; acc=int(pred.sum()); prec=float((pred & y).sum()/max(acc,1)); rec=float((pred & y).sum()/max(int(y.sum()),1))
            metric=None
            if acc>0:
                records,opened=build(native,metas,oof,float(thr))
                payload=dict(native); payload['records']=records; payload['model_name']=f'v6a3_{name}_{args.target}_thr{thr:.3f}'
                cache=outdir/f'v6a3_{name}_{args.target}_thr{thr:.3f}.pt'; torch.save(payload,cache)
                std=eval_records(records); aj=run_ajrd(cache,outdir/f'v6a3_{name}_{args.target}_thr{thr:.3f}_ajrd.json')
        ...
        results[name]={'ap':ap_score,'auc':auc,'folds':folds,'oof_path':str(outdir/f'v6a3_{name}_{args.target}_oof.npy'),'sweeps':sweeps,'best_constrained':best_constrained,'best_raw':best_raw}
    summary={'script':'scripts/train_cotracker3_online_v6a3_reentry_verifier.py','labels':str(args.labels),'target':args.target,'n_samples':int(len(y)),'n_pos':int(y.sum()),'pos_rate':float(y.mean()),'native_cache':str(args.native_cache),'native_row':native_row,'feature_names':names,'leakage_audit_passed':True,'models':results}
    out=outdir/f'v6a3_reentry_verifier_{args.target}_loov_summary.json'; out.write_text(json.dumps(summary,indent=2,ensure_ascii=False))
    print(json.dumps({'out':str(out),'target':args.target,'n_samples':summary['n_samples'],'n_pos':summary['n_pos'],'pos_rate':summary['pos_rate'],'model_brief':{k:{'ap':v['ap'],'auc':v['auc'],'best_constrained':v['best_constrained'][:3],'best_raw':v['best_raw'][:3]} for k,v in results.items()}},indent=2,ensure_ascii=False))
