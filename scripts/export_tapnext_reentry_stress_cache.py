...
def make_record(src: Dict[str, Any], pred_tracks_xy_tn: torch.Tensor, pred_vis_tn: torch.Tensor, idx: int) -> Dict[str, Any]:
    ...
    rec = {
        ...
        'adapter_version': 'tapnext_reentry_stress_v1',
        'raw_coordinate_note': 'TAPNext output xy pixel; converted to unified normalized yx by original_size [H-1,W-1]. Queries are [t,y,x] normalized.',
        'model_name': 'tapnext_bootstapnext_reentry_stress',
    }
...
def main() -> None:
    ...
    ap.add_argument('--max-queries', type=int, default=0, help='0 means all queries; smoke can use e.g. 256')
    args = ap.parse_args()
    ...
    if not torch.cuda.is_available():
        raise RuntimeError('CUDA is required for TAPNext export')
    ...
    for idx, src0 in enumerate(records_in):
        ...
        video = torch.from_numpy(video_np).permute(0,3,1,2).float().unsqueeze(0).cuda(non_blocking=True)  # B,T,C,H,W, 0..255
        q = torch.from_numpy(npy(src['query_points']).astype(np.float32)).unsqueeze(0).cuda(non_blocking=True) # B,N,[t,y_norm,x_norm]
        ...
        info = {
            'idx': idx + lo,
            'video_id': vid,
            'frames': int(video_np.shape[0]),
            'queries': int(npy(src['query_points']).shape[0]),
            'size_hw': [h,w],
            'sec': round(sec, 3),
    ...
    out = {
        ...
        'source_stress_dataset': args.stress_dataset,
        'stress_name': payload.get('stress_name'),
