...
OUTDIR = Path('outputs/paper_discovery_2026-06-27/rgb_stacking_fresh20_49_natural_ablation')
...
def main() -> None:
    ...
    base = torch.load(OFFLINE, map_location='cpu', weights_only=False)
    over = torch.load(ONLINE, map_location='cpu', weights_only=False)
    check_alignment(base, over)
    specs = [
        ('offline', None),
        ('online_global', None),
        ('b2_fullpost_p1', {'W': None, 'persist': 1, 'pre': 1}),
        ('b2_w8_p2', {'W': 8, 'persist': 2, 'pre': 1}),
        ('b2_w16_p1', {'W': 16, 'persist': 1, 'pre': 1}),
        ('b2_w16_p2', {'W': 16, 'persist': 2, 'pre': 1}),
        ('b2_w32_p2', {'W': 32, 'persist': 2, 'pre': 1}),
    ]
    rows = []
    trigger_stats = {}
    for name, params in specs:
        if name == 'offline':
            ...
            path = OUTDIR / f'{name}_rgb_fresh20_49.pt'
            trigger_stats[name] = build_variant(base, over, path, name=f'{name}_rgb_fresh20_49', **params)
    ...
    for r in rows:
        ...
        gains[f'{r["name"]}_vs_offline'] = {
            ...
            'delta_avg_256': round(float(r['delta_avg_256']) - float(by['offline']['delta_avg_256']), 6),
        }
    summary = {
        'split': 'rgb_fresh20_49',
        'source_caches': {'offline': str(OFFLINE), 'online': str(ONLINE)},
        'methods': rows,
        'gains': gains,
    ...
    for r in rows:
        compact.append({
            ...
            'AJ_256': r['AJ_256'],
            'OA_256': r['OA_256'],
            'delta_vs_offline': gains.get(f'{r["name"]}_vs_offline'),
        })
