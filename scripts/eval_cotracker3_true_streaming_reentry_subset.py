...
DEFAULT_OUTDIR = ROOT / 'outputs/paper_discovery_2026-07-05/cotracker3_true_streaming_subset_eval'
...
def delayed_recovery_mask(score_nt: np.ndarray, coords_yx_nt: np.ndarray, base_vis_nt: np.ndarray, *, tau_low: float, pre_window: int, max_jump_px: float, min_len: int):
    ...
    for q in range(N):
        for s, _e in find_visible_segments(base_vis_nt[q]):
...
def run_variant(model: CoTrackerOnlinePredictor, base_record: dict, video: torch.Tensor, queries_xy: torch.Tensor, variant: str, params: dict):
        ...
        for ind in range(0, T - model.step, model.step):
            ...
            if variant == 'state_writeback':
                ...
                if len(q_idx) > 0:
                    ...
                    model.model.online_vis_predicted[0, tt, qq] = high_logit
                    model.model.online_conf_predicted[0, tt, qq] = high_logit
    ...
    if variant == 'output_only':
        open_mask_nt, output_stats = delayed_recovery_mask(score_nt, coords_yx_nt, pred_vis_nt, tau_low=params['tau_low'], pre_window=params['pre_window'], max_jump_px=params['max_jump_px'], min_len=params['min_len'])
...
def main():
    ...
    ap.add_argument('--state-prob', type=float, default=0.97)
    args = ap.parse_args()
    if not torch.cuda.is_available():
        raise RuntimeError('CUDA is required')
    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    params = {'tau_low': args.tau_low, 'pre_window': args.pre_window, 'max_jump_px': args.max_jump_px, 'min_len': args.min_len, 'state_prob': args.state_prob}
    ...
    variants = ['native', 'output_only', 'state_writeback']
    ...
    for variant in variants:
        ...
        payload['schema_version'] = 'cotracker3_true_streaming_reentry_subset_v1'
    ...
    for variant in ['output_only', 'state_writeback']:
    ...
    report = {
        'script': 'scripts/eval_cotracker3_true_streaming_reentry_subset.py',
        'params': params,
    ...
    out_report = outdir / 'cotracker3_true_streaming_reentry_subset_report.json'
