#!/usr/bin/env python3
"""Formal P0i joint LMRA + CMCP + comparator training."""
...
DEFAULT_OUTPUT = REPO_ROOT / "outputs/routeD_cmcp_lmra_training_20260717/seed17"
...
def main() -> None:
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": adapter.parameters(), "lr": training["lmra_learning_rate"]},
            {"params": cmcp.parameters(), "lr": training["cmcp_learning_rate"]},
            {"params": comparator.parameters(), "lr": training["comparator_learning_rate"]},
        ],
    ...
    gate["decision"] = (
        ...
        if gate["pass"]
        else "CLOSE_STRONG_BACKBONE_RESCUE_ROUTE"
    )
    ...
    bundle = {
        "schema_version": "routeD_cmcp_lmra_joint_training_bundle_v0",
        "seed": args.seed,
        ...
        "optimizer": {
            "type": "AdamW",
            "lmra_learning_rate": training["lmra_learning_rate"],
            "cmcp_learning_rate": training["cmcp_learning_rate"],
