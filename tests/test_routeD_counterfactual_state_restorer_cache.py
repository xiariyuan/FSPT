...
def test_artifact_verification_detects_hash_tamper(tmp_path: Path):
    ...
    model_tensors = {
        ...
        "native_commit_coordinates_xy": torch.zeros(2, 2, dtype=torch.float16),
        "teacher_commit_coordinates_xy": torch.zeros(2, 2, dtype=torch.float16),
        "native_visibility_logits": torch.zeros(2, dtype=torch.float16),
        "native_confidence_logits": torch.zeros(2, dtype=torch.float16),
        "teacher_visibility_logits": torch.zeros(2, dtype=torch.float16),
        "teacher_confidence_logits": torch.zeros(2, dtype=torch.float16),
